...
const normalizeProviderConfig = (config: unknown): Required<Pick<AiProviderConfig, 'provider' | 'model'>> & AiProviderConfig => {
  ...
  const defaultModel =
    provider === 'azure' || provider === 'bedrock' ? '' : DEFAULT_MODELS[provider];
